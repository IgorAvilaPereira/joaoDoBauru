...
$$
BEGIN
     RETURN QUERY SELECT pedido.id, pedido.data_hora, STRING_AGG(produto.descricao || ',' || item.quantidade || ',' || item.valor_atual, ';') items FROM pedido inner join item on(pedido.id = item.pedido_id) inner join produto on (item.produto_id = produto.id) group by pedido.id, pedido.data_hora; 
END;
$$ LANGUAGE 'plpgsql';
